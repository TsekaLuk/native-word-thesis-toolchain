...
- Headers: if the target WPS/Word template visually uses literal spaces rather than a tab between the school label and `第 X 页 共 Y 页`, count and preserve the exact spacer string from the accepted rendering, then re-check in the same target Office engine. Pages/PDF export may wrap long literal-space headers differently, so the target engine decides.
