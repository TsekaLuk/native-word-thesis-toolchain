...
- Math typography: do not leave OMML math font unspecified, but validate in the target Office engine before choosing an aggressive font. In WPS delivery, `STIX Two Math` can make normal text inside fractions disappear; prefer stable `Cambria Math` for generated OMML, set both `settings.xml` `m:mathFont` and direct `w:rPr/w:rFonts` on every `m:r`, then guard against missing or mismatched direct math-run fonts.
