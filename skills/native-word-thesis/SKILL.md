...
- Math operators: WPS can drop standalone OMML operator runs such as ` = ` or ` + ` when they carry literal surrounding spaces and a math-symbol font. Emit operator runs without literal spaces (`=`, `+`, `×`, `≥`) and bind them to a stable text font such as `Times New Roman`; let the equation engine handle math spacing. Add a guard that rejects spaced operator runs and mismatched operator fonts.
...
- Style residue: do not let Pandoc/Markdown residue styles such as `Compact`, `FirstParagraph`, or `CaptionedFigure` remain on real body paragraphs. Map ordinary paragraphs to the configured thesis body style and table-cell paragraphs to a hidden native `TableBody`/`表格正文` style so WPS's style gallery and later manual edits stay predictable.
...
- Lists of tables/figures from LaTeX: `.lof`/`.lot` commonly contain chapter grouping commands such as `\addvspace{10pt}`. Locally mask `\addvspace` inside the `\listoffigures`/`\listoftables` wrapper so front-matter lists have no unexplained blank rows, but do not change global TOC behavior. Do not write `附图清单`/`附表清单` back into the main TOC unless the handbook explicitly requires it.
...
- Cover labels: when a cover label needs spaced characters such as `学    院：` or `学  号：`, use non-breaking spaces or character spacing, not ordinary ASCII spaces. Ordinary spaces can wrap inside narrow label cells and split `学号` across two lines even when the cell has `w:noWrap`.
...
- Generated charts: text inside generated academic figures must survive downscaling in Word/PDF. For body-width charts, render at high resolution and size CJK/Latin labels generously before insertion; then inspect the rendered page, not just the source PNG.
