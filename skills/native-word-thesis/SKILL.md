...
- Math typography: do not leave OMML math font unspecified. Pick a real installed math font for the delivery environment; for JOU-style macOS/WPS output, `STIX Two Math` gives a sharper LaTeX-like academic feel than fallback SimSun/Times. Set both `settings.xml` `m:mathFont` and direct `w:rPr/w:rFonts` on every `m:r`, then guard against missing direct math-run fonts.
